"""Transit-window analysis of a real TESS SPOC light curve for HAT-P-32 b.
...
DATA_FILE = DATA_DIR / "tess2022302161335-s0058-0000000292152376-0247-s_lc.fits"
PLANET = "HAT-P-32 b"
PERIOD_DAYS = 2.1500082
EPOCH_BJD = 2455867.402743
DURATION_HOURS = 3.12048
SECTOR = 58
FIGURE_FILE = FIG_DIR / "hatp32b_tess_transit.png"
...
def main() -> dict[str, float]:
    ...
    ax.scatter(hours, (result["flux"] - 1) * 1e6, s=5, alpha=0.12, color="#f08c00", label="quality-filtered cadences")
    ...
    ax.plot(hours[order], (result["model"][order] - 1) * 1e6,
            color="#f08c00", linewidth=2.2, label="fitted box model")
    ax.axvspan(-DURATION_HOURS / 2.0, DURATION_HOURS / 2.0, color="#f08c00", alpha=0.08)
